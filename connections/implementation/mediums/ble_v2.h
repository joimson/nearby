...
class BleV2 final {
 ...
 private:
  ...
  struct GattAdvertisementInfo {
    ...
    bool Exists(int slot) const { return gatt_advertisements.contains(slot); }
  ...
  absl::flat_hash_set<::nearby::cal::api::GattCharacteristic>
      gatt_characteristics_ ABSL_GUARDED_BY(mutex_);
